...
const parseMaxFileSize = (value: string | undefined): number => {
  ...
  if (!Number.isSafeInteger(parsed) || parsed <= 0) {
    throw new AppError(
      'IMAGEKIT_MAX_FILE_SIZE_BYTES must be a positive integer',
      500,
    );
  }
...
export const getImageKitConfig = (): ImageKitConfig => {
  ...
  if (!privateKey) {
    throw new AppError(
      'ImageKit configuration is incomplete. Set IMAGEKIT_PRIVATE_KEY',
      500,
    );
  }
  ...
  return {
    privateKey,
    maxFileSizeBytes: parseMaxFileSize(
      process.env.IMAGEKIT_MAX_FILE_SIZE_BYTES,
    ),
  };
};
