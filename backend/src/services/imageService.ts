...
import { getImageKitConfig, imageKitClient } from "../config/imageKit.js";
...
const detectImageMimeType = (file: Buffer): string | null => {
    ...
    if (file.length >= 6 && file.subarray(0, 6).toString("ascii") === "GIF87a") {
        return "image/gif";
    }

    if (file.length >= 6 && file.subarray(0, 6).toString("ascii") === "GIF89a") {
        return "image/gif";
    }
...
const validateImageUpload = (input: ImageUploadInput, maxFileSizeBytes: number): void => {
    ...
    if (!ALLOWED_IMAGE_TYPES.has(declaredMimeType)) {
        throw new AppError("Unsupported image type. Allowed types: JPEG, PNG, WebP, GIF", 400);
    }

    if (input.file.length > maxFileSizeBytes) {
        throw new AppError(`Image file exceeds the ${maxFileSizeBytes}-byte size limit`, 400);
    }

    if (!SAFE_FILE_NAME.test(input.fileName) || input.fileName.includes("..")) {
        throw new AppError("Image file name contains unsupported characters", 400);
    }

    const detectedMimeType = detectImageMimeType(input.file);

    if (!detectedMimeType) {
        throw new AppError("Unable to verify the image file type", 400);
    }

    if (detectedMimeType !== declaredMimeType) {
        throw new AppError("Image file type does not match its declared MIME type", 400);
    }
};

const toUploadedImage = (response: ImageKit.FileUploadResponse, input: ImageUploadInput): UploadedImage => {
    if (!response.fileId || !response.url || !response.name || response.size === undefined) {
        throw new AppError("ImageKit returned an incomplete upload response", 502);
    }
...
export const uploadImage = async (input: ImageUploadInput): Promise<UploadedImage> => {
...
export const deleteImage = async (fileId: string): Promise<void> => {
    if (!fileId || !/^[a-zA-Z0-9_-]+$/.test(fileId)) {
        throw new AppError("A valid ImageKit file ID is required", 400);
    }
