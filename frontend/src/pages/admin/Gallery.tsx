...
const ERA_FILTER_OPTIONS = [
    { value: "all", label: "All Eras (1920–2026)" },
    { value: "1920-1939", label: "1920–1939 (Foundation)" },
    { value: "1940-1959", label: "1940–1959 (Post-Independence)" },
    { value: "1960-1979", label: "1960–1979 (Golden Era)" },
    { value: "1980-1999", label: "1980–1999 (Modern Revival)" },
    { value: "2000-2026", label: "2000–Present" },
];

const CATEGORY_FORM_OPTIONS = GALLERY_CATEGORIES.map((cat) => ({
    value: cat,
    label: cat,
}));
...
export default function AdminGallery() {
    ...
    const taggablePlayerOptions = useMemo(() => {
        return players
            .filter((p) => !taggedPlayerIds.includes(p._id))
            .map((p) => ({
                value: p._id,
                label: p.name,
                sublabel: p.jerseyNumber ? `Jersey #${p.jerseyNumber}` : undefined,
            }));
        ...
        <div className="flex flex-col min-h-screen bg-[#F4F1EA]">
            ...
            <header className="px-6 md:px-8 py-6 border-b border-[rgba(26,26,26,0.08)] bg-[#FCF9F2] flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs md:text-sm text-[#6B665F] mt-1">Manage archival photographs and media.</p>
                </div>
                        ...
                        {selectedItem && (
                            <div className="p-4 bg-[#ECE8E1] border-b border-[rgba(26,26,26,0.08)] flex items-center">
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-[#E2DDD4] text-[#3d030b] border border-[#3d030b]/20 tracking-wider uppercase font-mono">
                                    EDITING MEDIA — ID: #MED-{selectedItem._id.slice(-6).toUpperCase()}
                                </span>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="p-6 flex flex-col gap-6">
                                    ...
                                    {previewUrl ? (
                                        <div className="w-full h-44 bg-[#E2DDD4] overflow-hidden rounded relative border border-[rgba(26,26,26,0.1)]">
                                            ...
                                            <button
                                                ...
                                                onClick={() =>
                                                    setZoomItem(
                                                        selectedItem || ({ imageUrl: previewUrl } as GalleryItem),
                                                    )
                                                }
                            ...
                            <div className="space-y-3 bg-[#FCF9F2] p-4 border border-[rgba(26,26,26,0.1)] rounded">
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <AdminSelect
                                        ...
                                        placeholder="None / Institutional Archival Only"
                                        searchable={true}
                                    />
                                </div>
                            ...
                            <div className="space-y-3 bg-[#FCF9F2] p-4 border border-[rgba(26,26,26,0.1)] rounded">
                                ...
                                <div className="pt-1">
                                    <AdminSelect
                                        ...
                                        options={taggablePlayerOptions}
                                        placeholder="Search & tag player from roster..."
                                        searchable={true}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="pt-4 border-t border-[rgba(26,26,26,0.08)] flex items-center justify-end space-x-3">
                                ...
                                >
                                    ...
                                    <span>{selectedItem ? "Save Changes" : "Commit Record"}</span>
                                </button>
                            </div>
                        </form>
                    </div>
